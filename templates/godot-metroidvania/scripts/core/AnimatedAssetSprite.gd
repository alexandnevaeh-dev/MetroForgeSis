...
## this asset" (expected for enemies/bosses, which don't have per-state sheets yet) — that's
## treated as normal, not an error. A non-empty path pointing at a file that doesn't exist on
## disk falls back to a solid-color placeholder with a warning, not a crash.
